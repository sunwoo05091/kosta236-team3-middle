...
<table class="table table-hover" style="width :70%; margin-left: 350px; text-align: center;">
  ...
  <tbody>
	...
	<tr class="table-light">
		...
		<td><a href="detailCarfullAction.do?cm_no=${community.cm_no}">${community.cm_title }</a>
...
<a class="badge bg-success" href="insertCommunityCarfullFormAction.do" style="margin-left: 1450px;">글쓰기</a>
	...
	<div class="pageBox">
		...
		<c:if test="${listModel.startPage >= 6 }">
			<li class="page-item active">
				<a class="page-link" href="listCommunityCarfullAction.do?pageNum=${listModel.startPage - 1 }">[이전]</a>
			</li>	
		</c:if>
		...
		<c:forEach var="pageNo" begin="${listModel.startPage }" end="${listModel.endPage }">
			<c:if test="${listModel.requestPage == pageNo }"><b></c:if>
				 <li class="page-item active">
					<a class="page-link" href="listCommunityCarfullAction.do?pageNum=${pageNo }">[${pageNo }]</a>
				 </li>	
			<c:if test="${listModel.requestPage == pageNo }"></b></c:if>
		</c:forEach>
		...
		<c:if test="${listModel.endPage < listModel.totalPageCount }">
			<li class="page-item active">
				<a class="page-link" href="listCommunityCarfullAction.do?pageNum=${listModel.startPage + 5 }">[이후]</a>
			</li>	
		</c:if>
		</ul>
	</div>
