...
<head>
    ...
    <script>
        function referAnnualLeave(frm) {
            frm.method = "post";
            frm.action = '${sessionScope.contextPath}/mypage/referAnnualLeave.do';
            frm.submit();
            return true;
        }
        ...
        function acknowledgeAnnualLeave(frm) {
            frm.method = "post";
            frm.action = '${sessionScope.contextPath}/mypage/acknowledgeAnnualLeave.do';
            frm.submit();
            return true;
        }
        ...
        function onProgressAnnualLeave(frm){
            frm.method = "post";
            frm.action = '${sessionScope.contextPath}/mypage/onProgressAnnualLeave.do';
            frm.submit();
            return true;
        }
    </script>
...
<a class="badge rounded-pill bg-primary add" href="${sessionScope.contextPath}/mypage/insertAnnualLeave.do">추가</a>
<ul>
    ...
    <ul class="pagination">
        <c:if test="${listAnnualLeave.startPage >= 6}">
            <li class="page-item disabled">
                <a class="page-link" href="annual-leave.do?pageNum=${listAnnualLeave.startPage - 1}">&laquo;</a>
            </li>
        </c:if>
            ...
            <c:choose>
                <c:when test="${listAnnualLeave.requestPage == pageNo}">
                    <li class="page-item active">
                        <a class="page-link" href="annual-leave.do?pageNum=${pageNo}">${pageNo}</a>
                    </li>
                </c:when>
                <c:otherwise>
                    <li class="page-item">
        ...
        <c:if test="${listAnnualLeave.endPage < listAnnualLeave.totalPageCount}">
            <li class="page-item">
                <a class="page-link" href="annual-leave.do?pageNum=${listAnnualLeave.endPage + 1}">&raquo;</a>
            </li>
        </c:if>
    </ul>
</div>
