...
public class ListPayStub implements Action {
    public ActionForward execute(HttpServletRequest request, HttpServletResponse response)
        ...
        MyPage_ListDB listDB = myPageService.listDB("PayStub", userNo, request);
        ...
        if (Integer.parseInt(presentYear) == Integer.parseInt(year) && Integer.parseInt(presentMonth) == Integer.parseInt(month)) {
            ...
            for (int i = 0; i < listDB.getList().size(); i++) {
                String db_year = ((List<MyPage_PayStub>)listDB.getList()).get(i).getP_Date().substring(0, 4);
                String db_month = ((List<MyPage_PayStub>)listDB.getList()).get(i).getP_Date().substring(5, 7);
                String db_day = ((List<MyPage_PayStub>)listDB.getList()).get(i).getP_Date().substring(8, 10);
            ...
            if (HaveToMakePayStub == true) {
                ...
                if (listDB.getList().size() != 0){
                    nextP_No = ((List<MyPage_PayStub>)listDB.getList()).get(0).getP_No() + 1;
                }
                int e_No = privateInfo.getE_No();
//                String p_Date = formattedNow;
                String p_Date = presentYear + "/" + presentMonth + "/" + "10";
                MyPage_PayStub myPagePayStub = new MyPage_PayStub(nextP_No, e_No, p_Date, p_basePay);
