...
import kosta.action.SchedulePageAction;
import kosta.action.TypeScheduleAction;
...
public class ScheduleController extends HttpServlet {
    ...
    public void doProcess(HttpServletRequest request, HttpServletResponse response)throws ServletException, IOException{
    	...
    	}else if(command.equals("TypeScheduleAction.do")) {
    		action = new TypeScheduleAction();
    		try {
				forward = action.execute(request, response);
			} catch (Exception e) {
				e.printStackTrace();
			}
    	}else if(command.equals("SchedulePageAction.do")) {
    		action = new SchedulePageAction();
    		try {
				forward = action.execute(request, response);
			} catch (Exception e) {
				e.printStackTrace();
